...
async function createMintSignature(serverWallet, wallet, hashes, nonce, contractAddress) {
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const signature = await createMintSignature(serverWallet, wallet, hashes, state.currentNonce, CONTRACT_ADDRESS);
    const data = encodeMintData(wallet, hashes, state.currentNonce, signature);
    const gasLimit = await estimateGasLimit(provider, wallet, CONTRACT_ADDRESS, data, state.mintPrice);
