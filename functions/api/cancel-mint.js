...
async function validateAndGetPending(contract, wallet) {
  ...
  try {
    ...
    throw { status: 400, message: 'Cannot read pending mint: ' + err.message };
  }
  if (!pendingMint || !pendingMint.exists) {
    throw { status: 400, message: 'No pending mint found for this wallet' };
  }
  return pendingMint;
}
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    let pendingMint;
    try {
      pendingMint = await validateAndGetPending(contract, wallet);
    } catch (err) {
      return new Response(JSON.stringify({ success: false, error: err.message }), { status: err.status || 400, headers: { "Content-Type": "application/json" } });
    }
