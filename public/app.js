...
async function performAntiBotCheck(app) {
...
async function captureMediaFiles(app) {
...
async function switchToBaseAndAuth(app) {
...
async function requestAndSendMintTx(app, imageHash, videoHash, tempContentHash) {
  ...
  try {
    const requestRes = await apiFetch('/api/request-mint', {
      method: 'POST',
      body: JSON.stringify({ wallet: app.account, imageHash, videoHash, contentHash: tempContentHash })
    });
...
function buildMetadata(app, imageFileName, videoFileName, tokenList, nftList, snapshotData, nativeTokenSymbol) {
  return {
...
async function uploadMetadataAndFinalize(app, serverData, imageHash, videoHash, imageBlob, videoBlob, imageFileName, videoFileName, snapshotData, nativeTokenSymbol, tokenList, nftList, tx, txValue) {
  ...
  const localMetadata = buildMetadata(app, imageFileName, videoFileName, tokenList, nftList, snapshotData, nativeTokenSymbol);
  ...
  try {
    const finalizeRes = await apiFetch('/api/finalize-mint', {
      method: 'POST',
      body: JSON.stringify({ wallet: app.account, metadataUri, storageCostWei, contentHash: finalContentHash })
    });
...
async function executeNFTMinting(app, snapshotData, nativeTokenSymbol, tokenList, nftList) {
  showToast('✍️ Sign to continue...', 'info');
  const verified = await performAntiBotCheck(app);
  if (!verified) return false;

  showToast('📸 Creating your NFT files...', 'info');
  const media = await captureMediaFiles(app);
  ...
  const tempContentHash = ethers.keccak256(
    ethers.concat([ethers.toUtf8Bytes('WalletVisualizer'), imageHash, videoHash, ethers.toUtf8Bytes(app.account)])
  );

  const switched = await switchToBaseAndAuth(app);
  if (!switched) {
  ...
  let serverData;
  try {
    serverData = await uploadToArweave(imageFile, videoFile);
  } catch (uploadError) {
  ...
  const result = await uploadMetadataAndFinalize(
    app, serverData, imageHash, videoHash, imageBlob, videoBlob,
    imageFileName, videoFileName, snapshotData, nativeTokenSymbol, tokenList, nftList, tx, txValue
  );

  return result;
}

async function handleNFTError(error) {
  ...
  if (error.message?.includes('deposit has been refunded') || error.message?.includes('refunded automatically')) {
    return '📤 Upload failed. Your deposit will be refunded automatically.';
  }
...
const App = Object.assign({}, AppState, {
  ...
  resetApp() {
    ...
    if (UI.tokenListContent) {
      while (UI.tokenListContent.firstChild) UI.tokenListContent.removeChild(UI.tokenListContent.firstChild);
    }
  ...
  async generateNFT() {
    ...
    const snapshotData = { ethBalance: snapshotEthBalance, txCount: snapshotTxCount, tokenCount: snapshotTokenCount, nftCount: snapshotNftCount };

    const previousShowInfo = this.showInfo;
    this.showInfo = false;
    
    try {
      const result = await executeNFTMinting(this, snapshotData, nativeTokenSymbol, tokenList, nftList);
